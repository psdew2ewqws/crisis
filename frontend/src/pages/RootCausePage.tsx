    ...
    <div className="flex-1 overflow-y-auto bg-bg">
      <div className="mx-auto max-w-[1340px] px-8 py-7">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[28px] font-semibold tracking-tight text-txt">{t('Root Cause Analysis')}</h1>
            <p className="mt-1.5 flex items-center gap-2 text-[14px] text-muted">
              <Database className="h-3.5 w-3.5" />
              {t('Ranked RIL problem clusters from voc360')}
              {causes.length > 0 && (
                <span className="text-faint">
                  · {t('{clusters} clusters · {reports} citizen reports', { clusters: causes.length, reports: totalMembers.toLocaleString() })}
                </span>
              )}
            </p>
          </div>
        ...
        {err && (
          <div className="mt-6 flex items-center gap-2 rounded-lg border border-danger/40 bg-card px-4 py-3 text-[13px] text-danger">
            ...
            {t('Could not load root causes — {err}', { err })}
          </div>
        )}
        ...
        {!err && !loading && causes.length === 0 && (
          <div className="mt-10 rounded-xl border border-border bg-card px-5 py-8 text-center text-[13px] text-muted">
            {t('No root-cause clusters returned by voc360.')}
          </div>
        )}
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_400px]">
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="border-b border-border px-5 py-3 font-mono text-[10px] tracking-[0.14em] text-faint">
                {t('RANKED ROOT CAUSES · RIL CLUSTERS')}
              </div>
              <ul>
                {causes.map((c) => {
  ...
  if (!cause) {
    return (
      <aside className="rounded-xl border border-border bg-card p-5 text-[13px] text-muted">
        {t('Select a cluster to inspect its evidence and recommended countermeasure.')}
      </aside>
    )
  }
  ...
  return (
    <aside className="flex flex-col gap-4 self-start rounded-xl border border-border bg-card p-5">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] tracking-[0.14em] text-faint">
            {t('CLUSTER · RANK #{rank}', { rank: cause.rank })}
          </span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <Metric label={t('REPORTS')} value={cause.members.toLocaleString()} />
        <Metric label={t('SEVERITY')} value={String(cause.severity_avg)} tone={col} sub={t(sevLabel(cause.severity_avg))} />
        <Metric label={t('SCORE')} value={String(cause.score)} />
      </div>
      ...
      <div>
        ...
        {cause.evidence.length > 0 ? (
          ...
          <p className="text-[12px] text-muted">{t('No sample segments returned for this cluster.')}</p>
        )}
      </div>
        ...
        <div className="rounded-lg border border-good/30 bg-good/10 p-3">
          ...
          {(solution.feasibility != null || solution.expected_impact != null || solution.owning_service) && (
            <dl className="mt-2 space-y-1 text-[11.5px]">
              {solution.owning_service && <SolutionRow k={t('Owner')} v={solution.owning_service} />}
              {solution.feasibility != null && (
                <SolutionRow k={t('Feasibility')} v={`${solution.feasibility_label} (${Math.round(solution.feasibility * 100)}%)`} />
              )}
              {solution.expected_impact != null && (
                <SolutionRow k={t('Expected impact')} v={`${solution.impact_label} (${Math.round(solution.expected_impact * 100)}%)`} />
              )}
            </dl>
          )}
        </div>
      ...
      >
        <Network className="h-4 w-4" />
        {t('View this cluster in the graph')}
      </button>
    </aside>
  )
}
