        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1.5 text-[14px] text-muted">
              {t('Voice-of-Customer signal layer · {table} · {total} records', {
                table: 'the_data',
                total: loading && !data.rows.length ? '…' : total.toLocaleString(),
              })}
            </p>
          </div>
          <button
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2.5">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              placeholder={t('Search signal text…')}
              dir="auto"
        ...
        <div className="mt-4 overflow-hidden rounded-xl border border-border bg-card">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[11px] font-medium tracking-[0.08em] text-faint">
                <th className="px-5 py-3 font-medium">{t('SEVERITY')}</th>
                <th className="py-3 font-medium">{t('SIGNAL')}</th>
                <th className="py-3 font-medium">{t('SERVICE')}</th>
                <th className="py-3 font-medium">{t('SOURCE')}</th>
                <th className="py-3 font-medium">{t('SENTIMENT')}</th>
                <th className="px-5 py-3 text-right font-medium">{t('OBSERVED')}</th>
              </tr>
            </thead>
            <tbody>
              {err && (
                <tr>
                  <td colSpan={6} className="px-5 py-10 text-center">
                    <div className="mx-auto flex max-w-md flex-col items-center gap-2 text-muted">
                      ...
                      <div className="text-[14px] text-txt">{t('Could not load signals')}</div>
              ...
              {!err && loading && data.rows.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-muted">
                    ...
                    <div className="mt-2 text-[13px]">{t('Loading voc360 signals…')}</div>
                  </td>
                </tr>
              )}
              ...
              {!err && !loading && data.rows.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-muted">
                    ...
                    <div className="mt-2 text-[14px] text-txt">{t('No signals match these filters')}</div>
                    <div className="mt-1 text-[13px] text-faint">{t('Try clearing a filter or the search term.')}</div>
                  </td>
                </tr>
              )}
          ...
          <div className="flex items-center justify-between border-t border-border px-5 py-3">
            <span className="text-[12.5px] text-muted">
              {total === 0 ? t('No results') : `${from.toLocaleString()}–${to.toLocaleString()} of ${total.toLocaleString()}`}
            </span>
            <div className="flex items-center gap-2">
              <span className="text-[12.5px] text-faint">
                {t('Page {n} / {m}', { n: q.page + 1, m: pageCount })}
              </span>
              <button
