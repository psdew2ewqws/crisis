  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      <div className="mb-4 flex items-start justify-between">
        <div>
          <div className="text-[15px] font-semibold text-txt">{t(metric.title)}</div>
          <div className="mt-0.5 text-[12.5px] text-muted">{t(metric.sub)}</div>
        </div>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={data} margin={{ top: 6, right: 8, bottom: 0, left: -16 }}>
            ...
            <Tooltip
              ...
              labelFormatter={(l) => t('Step {l}', { l })}
              formatter={(v, name) => [metric.format(num(v)), name === 'before' ? t('Before') : t('After fix')]}
            />
            <Area
              type="monotone"
              dataKey="before"
...
export default function SimulationPage({ caseId }: { caseId?: string } = {}) {
  ...
  const load = useCallback(() => {
    ...
    getSimulate(caseId)
      .then((s) => setSim(s))
      .catch((e: unknown) => setError(e instanceof Error ? e.message : t('Simulation failed')))
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto max-w-[1340px] px-8 py-7">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[28px] font-semibold tracking-tight text-txt">{t('Simulation')}</h1>
            <p className="mt-1.5 text-[14px] text-muted">
              {(() => {
                const boldText = t('before vs after')
                const [before, after] = t(
                  'Agent-based sentiment propagation across the live voc360 graph · {bold} the root-cause fix',
                  { bold: boldText },
                ).split(boldText)
                return (
                  <>
                    {before}
                    <span className="font-medium text-txt">{boldText}</span>
                    {after}
                  </>
                )
              })()}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {loading ? t('Simulating…') : t('Re-run simulation')}
            </button>
          </div>
        </div>
        ...
        {error && (
          <div className="mt-6 flex items-center gap-2.5 rounded-xl border border-danger/30 bg-danger/10 px-4 py-3 text-[13px] text-danger">
            ...
            <span>{t('Could not reach the simulation engine: {err}', { err: error })}</span>
          </div>
        )}
        ...
        {loading && !hasResult && !error && (
          <div className="mt-16 flex flex-col items-center justify-center gap-3 text-muted">
            ...
            <span className="text-[13.5px]">{t('Building case graph and propagating sentiment…')}</span>
          </div>
        )}
        ...
        {!loading && !hasResult && !error && (
          <div className="mt-16 flex flex-col items-center justify-center gap-2 text-faint">
            ...
            <span className="text-[13.5px]">{t('No simulation result yet.')}</span>
          </div>
        )}
        ...
        {hasResult && (
          <>
            ...
            <div className="mt-6 rounded-xl border border-border bg-card p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-[12px] uppercase tracking-wide text-faint">{t('Intervention target')}</div>
                    {rcLabel ? (
                      <div className="mt-1 text-[16px] font-semibold text-txt" dir={isAr(rcLabel) ? 'rtl' : 'ltr'}>
                        {rcLabel}
                      </div>
                    ) : (
                      <div className="mt-1 text-[15px] font-medium text-muted">
                        {t('Top-ranked root-cause cluster (auto-targeted)')}
                      </div>
                    )}
                    <div className="mt-1 text-[12.5px] text-muted">
                      {rcMembers != null
                        ? t('{n} member segments damped at the source node', { n: fmt0(rcMembers) })
                        : t('Dominant cluster damped at the source node')}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <StatCard
                label={t('Δ Mean negativity')}
                value={signed(negDelta, fmtPct)}
                hint={t('{before} → {after} at final step', {
                  before: fmtPct(num(bLast?.mean_negativity)),
                  after: fmtPct(num(aLast?.mean_negativity)),
                })}
                tone={negDelta > 0 ? 'good' : negDelta < 0 ? 'danger' : 'neutral'}
                icon={TrendingDown}
              />
              <StatCard
                label={t('Δ Critical nodes')}
                value={signed(-critDelta, (n) => fmt0(Math.abs(n)))}
                hint={t('{before} → {after} nodes over threshold', {
                  before: fmt0(num(bLast?.n_critical)),
                  after: fmt0(num(aLast?.n_critical)),
                })}
                tone={critDelta > 0 ? 'good' : critDelta < 0 ? 'danger' : 'neutral'}
                icon={AlertTriangle}
              />
              <StatCard
                label={t('Δ Peak negativity')}
                value={signed(peakDelta, fmtPct)}
                hint={t('Reduction in worst-tick sentiment load')}
                tone={peakDelta > 0 ? 'good' : peakDelta < 0 ? 'danger' : 'neutral'}
                icon={Activity}
              />
              <StatCard
                label={t('Ticks to settle')}
                value={fmt0(settle)}
                hint={t('Steps until the fixed system stabilizes')}
                tone="blue"
                icon={GitBranch}
              />
            </div>
            ...
            {(params.steps != null ||
              ...
              <div className="mt-4 rounded-xl border border-border bg-card px-5 py-4">
                <div className="mb-3 text-[12px] uppercase tracking-wide text-faint">{t('Run parameters')}</div>
                <div className="flex flex-wrap gap-x-8 gap-y-2.5 font-mono text-[12.5px]">
                  <Param label={t('steps')} value={params.steps} format={fmt0} />
                  <Param label={t('spread_rate')} value={params.spread_rate} format={fmt2} />
                  <Param label={t('decay')} value={params.decay} format={(n) => n.toFixed(3)} />
                  <Param label={t('inflow')} value={params.inflow} format={(n) => n.toFixed(3)} />
                  <Param
                    label={t('intervention_strength')}
                    value={params.intervention_strength}
                    format={fmt2}
                  />
                  <Param label={t('seed')} value={params.seed} format={fmt0} />
                </div>
              </div>
            )}
...
function TrajectoryOverlay({ after }: { after: SimSeries[] }) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-border bg-card p-5">
      <div className="mb-4">
        <div className="text-[15px] font-semibold text-txt">{t('After-fix trajectory')}</div>
        <div className="mt-0.5 text-[12.5px] text-muted">
          {t('All three variables normalized to [0,1] over the post-intervention run')}
        </div>
      </div>
        ...
        <LineChart data={data} margin={{ top: 6, right: 8, bottom: 0, left: -20 }}>
          ...
          <Tooltip
            ...
            formatter={(v, name) => {
              const labels: Record<string, string> = {
                neg: t('Mean Negativity'),
                vol: `${t('Complaint Volume')} (norm)`,
                crit: `${t('Critical Nodes')} (norm)`,
              }
          ...
          <Legend
            ...
            formatter={(value) => {
              const labels: Record<string, string> = {
                neg: t('Mean Negativity'),
                vol: t('Complaint Volume'),
                crit: t('Critical Nodes'),
              }
