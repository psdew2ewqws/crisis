...
def _try_ollama_chat(messages: List[Dict[str, str]]) -> Optional[str]:
    ...
    out = _post_json(
        f"{LLM_BASE_URL}/api/chat",
        {
            ...
            "messages": messages,
            "stream": False,
            # Disable chain-of-thought: reasoning ("thinking") models would
            # otherwise spend the whole num_predict budget on hidden reasoning
            # and return empty content (done_reason=length). Ignored by
            # non-thinking models, so it is safe across any LLM_MODEL.
            "think": False,
    ...
    *,
    ...
    out = _post_json(
        ...
        {"model": LLM_MODEL, "messages": messages, "stream": False,
         "think": False,  # see _try_ollama_chat: keep reasoning models from
                          # eating num_predict on hidden thinking
