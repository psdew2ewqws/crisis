...
# ===========================================================================
# LIVE RSS NEWS  (GET /api/news)
# ===========================================================================
@router.get("/api/news")
def live_news(force: bool = Query(default=False)) -> Dict[str, Any]:
...
# ===========================================================================
_DOMAIN_KW_NEWS: Dict[str, List[str]] = {
    "water":      ["ماء", "مياه", "شُح", "جفاف", "خزان", "سدّ"],
    "health":     ["صحة", "مستشفى", "طوارئ", "وباء", "مرض", "وفاة"],
    "transport":  ["طريق", "حادث", "مرور", "ازدحام", "حافلة"],
...
def _classify_news_domain(texts: List[str]) -> str:
    blob = " ".join(texts).lower()
    scores: Dict[str, int] = {}
    for dom, kws in _DOMAIN_KW_NEWS.items():
...
def news_analysis(gov: str = Query(...)) -> Dict[str, Any]:
    """Composite analysis: recent news + signals + scenario pre-fill text."""
    empty: Dict[str, Any] = {"gov": gov, "articles": [], "article_count": 0,
    ...
    try:
        art_rows = db.fetchall(
            ...
            "FROM aegis_news WHERE gov = %(gov)s ORDER BY published DESC NULLS LAST LIMIT 20",
            {"gov": gov},
        )
        ...
        sev_rows = db.fetchall(
            f"select coalesce(severity,'unknown') sev, count(*) n from the_data {where} group by sev", bind)
        by_severity = {r["sev"]: int(r["n"]) for r in (sev_rows or [])}
        titles = [a.get("title", "") for a in articles]
        top_domain = _classify_news_domain(titles)
        ...
        if articles:
            ...
            scenario_text = (f"تصاعد الشكاوى في {gov_name}: {signal_total} إشارة"
                             + (f"، منها {crit} حالة حرجة" if crit else ""))
        else:
            scenario_text = f"وضع طارئ في محافظة {gov_name}"
        return {"gov": gov, "articles": articles, "article_count": len(articles),
                "domains": domains, "signal_total": signal_total, "by_severity": by_severity,
                "scenario_text": scenario_text, "generated_at": _now_iso()}
    except Exception as e:
